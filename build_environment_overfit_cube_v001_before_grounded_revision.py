"""Build a deterministic cube-only SE(3) planning environment around an existing Phobos robot.
...
def build_obstacle_specs(robot):
    ...
    current = robot["all_world"]
    floor_margin = margin + 0.02
    hole_min_x = max(ENV_BOUNDS_MIN[0] + 0.2, current["min"][0] - floor_margin)
    hole_max_x = min(ENV_BOUNDS_MAX[0] - 0.2, current["max"][0] + floor_margin)
    hole_min_y = max(ENV_BOUNDS_MIN[1] + 0.2, current["min"][1] - floor_margin)
    hole_max_y = min(ENV_BOUNDS_MAX[1] - 0.2, current["max"][1] + floor_margin)
    ...
    # Four cubes form a ground plane with a robot-sized center opening.  This keeps
    # the imported (partly below z=0) robot pose collision-free without moving it.
    add("env_floor_west_001", "FLOOR",
        ((ENV_BOUNDS_MIN[0] + hole_min_x) * 0.5, 0.0, -floor_thickness * 0.5),
        (hole_min_x - ENV_BOUNDS_MIN[0], 7.0, floor_thickness),
        "floor", "floor", ["cube_only", "ground", "current_pose_clearance"], group="floor")
    add("env_floor_east_001", "FLOOR",
        ((hole_max_x + ENV_BOUNDS_MAX[0]) * 0.5, 0.0, -floor_thickness * 0.5),
        (ENV_BOUNDS_MAX[0] - hole_max_x, 7.0, floor_thickness),
        "floor", "floor", ["cube_only", "ground", "current_pose_clearance"], group="floor")
    add("env_floor_south_001", "FLOOR",
        ((hole_min_x + hole_max_x) * 0.5, (ENV_BOUNDS_MIN[1] + hole_min_y) * 0.5, -floor_thickness * 0.5),
        (hole_max_x - hole_min_x, hole_min_y - ENV_BOUNDS_MIN[1], floor_thickness),
        "floor", "floor", ["cube_only", "ground", "current_pose_clearance"], group="floor")
    add("env_floor_north_001", "FLOOR",
        ((hole_min_x + hole_max_x) * 0.5, (hole_max_y + ENV_BOUNDS_MAX[1]) * 0.5, -floor_thickness * 0.5),
        (hole_max_x - hole_min_x, ENV_BOUNDS_MAX[1] - hole_max_y, floor_thickness),
        "floor", "floor", ["cube_only", "ground", "current_pose_clearance"], group="floor")

    pillar_cross = clamp(0.24 * horizontal_max, 0.26, 0.34)
    add("env_pillar_floor_001", "PILLARS", (-1.50, -1.16, 0.95),
        (pillar_cross, pillar_cross, 1.90), "pillar", "obstacle",
        ["cube_only", "floor_supported", "staggered"], group="pillars")
    ...
    add("env_pillar_hanging_001", "PILLARS", (-1.34, 0.10, 3.40),
        (0.30, 0.30, 1.20), "pillar", "beam",
        ["cube_only", "ceiling_suspended", "staggered"], group="pillars")
    add("env_pillar_hanging_002", "PILLARS", (-1.06, 1.92, 3.60),
        (0.26, 0.26, 0.80), "pillar", "beam",
        ["cube_only", "ceiling_suspended", "staggered"], group="pillars")
    ...
    add("env_center_block_001", "CENTRAL_OBSTACLES", (-0.05, 1.08, 0.90),
        (0.75, 0.58, 1.80), "central_block", "obstacle",
        ["cube_only", "floor_supported", "left_right_bypass", "overflight"], group="central")
    add("env_offset_pillar_001", "CENTRAL_OBSTACLES", (1.43, 1.62, 0.65),
        (0.26, 0.26, 1.30), "offset_pillar", "obstacle",
        ["cube_only", "floor_supported", "asymmetric_route"], group="central")

    add("env_beam_001", "BEAMS", (0.20, 2.63, 2.28),
        (1.20, 0.26, 0.24), "suspended_beam", "beam",
        ["cube_only", "overhead", "underpass", "overflight", "side_bypass"], group="beam_001")
    add("env_low_block_001", "BEAMS", (0.61, 2.28, 0.35),
        (0.55, 0.42, 0.70), "low_block", "beam",
    ...
    derived = {
        ...
        "floor_clearance_opening": {
            "min": [hole_min_x, hole_min_y, -floor_thickness],
            "max": [hole_max_x, hole_max_y, 0.0],
        },
    }
...
def write_environment_json(robot, specs, derived, validation, sample_min, sample_max):
    ...
    data = {
        ...
        "description": "Moderate difficulty cube-only SE3 planning environment",
        ...
        "derived_design": {
            ...
            "floor_clearance_opening": {
                "min": rounded(derived["floor_clearance_opening"]["min"]),
                "max": rounded(derived["floor_clearance_opening"]["max"]),
                "reason": "Preserves the current imported robot pose without modifying its transform",
            },
        },
        ...
        "passages": [
            ...
            },
            {
                "id": "passage_beam_001",
                "type": "suspended_beam",
                "clearance_size_xyz": [4.0, 0.30, 2.14],
                "difficulty": "medium",
        ...
        "validation": {
            ...
            **validation["free_space"],
            "connectivity_notes": [
                ...
                "The central floor opening exists only to keep the unchanged imported robot pose collision-free.",
            ],
        },
    }
