...
if ! "${runtime_python}" -c 'import coal' >/dev/null 2>&1; then
  ...
  if [[ ! -d "${coal_site}/coal" ]]; then
    echo "COAL is unavailable. Set HNUTER_COAL_ENV to a Python 3.12 conda environment containing coal." >&2
    exit 2
  fi
...
exec "${runtime_python}" "${project_dir}/run_overfit_cube_single_pipeline.py" "$@"
